...
# MATERIAL INDEX
mindex = mongo.db.data.find_one()['entry_id'].split('-')[0]
...
def thumbnails_information(max_entries=50, filter={}):
    ...
    if max_entries is None:
        ...
        entries = get_entries( [mindex+'-'+str(n) for n in nn] )
    else:
        total_count = get_total_count()
...
def random_selector():
    ...
    for i in range(total_count):
        st = get_structure( mindex+'-' + str(random.randint(4, total_count)) )
        ...
        if key == 'formula':
            data_list.append({'type': key,
                              ...
                              'formula': {'link':              comp.reduced_formula,
                                          'html': html_formula(comp.reduced_formula)},
                              'counts': count_format( mongo.db.data.count_documents({"data.system.reduced_formula": comp.reduced_formula}) )})
        else:
            data_list.append({'type': key,
                              'select_type': 'element',
